  ...
  useEffect(() => {
    const fetchWomenswearProducts = async () => {
      try {
        ...
        const WomenswearProducts = querySnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
          isLoading: true, // Individual loading state for each product
        }));
        console.log("Womenswear Products:", WomenswearProducts);
        setProducts(WomenswearProducts);

        // Simulate product loading individually
        WomenswearProducts.forEach((product, index) => {
          setTimeout(() => {
            setProducts((prev) =>
              prev.map((p) =>
        ...
        console.error("Error fetching Womenswear products: ", error);
      } finally {
