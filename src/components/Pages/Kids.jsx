...
const Kids = () => {
  ...
  useEffect(() => {
    const fetchKidswearProducts = async () => {
      try {
        const q = query(
          collection(db, "newproducts"),
          where("category", "==", "Kidswears")
        );
        const querySnapshot = await getDocs(q);

        const kidswearProducts = querySnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
          isLoading: true,
        }));

        setProducts(kidswearProducts);

        // Simulate a staggered loading effect
        kidswearProducts.forEach((product, index) => {
          setTimeout(() => {
            setProducts((prev) =>
              prev.map((p) =>
        ...
        console.error("Error fetching Kidswear products: ", error);
      } finally {
        setLoading(false);
      }
    };

    fetchKidswearProducts();
  }, []);
  ...
  return (
    <div className="min-h-[550px] sm:min-h-[650px] mt-20 relative overflow-hidden bg-gray-100 dark:bg-gray-950 dark:text-white">
      <div>
        <h1 className="m-6 font-bold text-2xl">Kids Wears</h1>
      </div>
      {/* Body Section */}
      <div>
        {loading ? (
